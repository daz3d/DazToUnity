...
*/
        ...
        public static IEnumerator ImportDTURoutine(string path, Action<DTU> dtuOut, float progressLimit)
        {
            ...
            for (int i = 0; i < dtu.Materials.Count; i++)
            {
                ...
                //// DB 2025-10-16: old incompatible key system:
                // var key = Utilities.ScrubKey(dtuMat.MaterialName);
                //// NEW system: dtuMat.SlotName (aka SanitizedModelName_SanitizedMaterialName)
                var key = dtuMat.SlotName;
        ...
        public static void GeneratePrefabFromFBX(string fbxPath, DazFigurePlatform platform, DTU dtu)
        {
            ...
            if (ReplaceMaterials)
            {
                foreach (var renderer in renderers)
                {
                    ...
                    {
                        foreach (var oMaterialSlot in renderer.sharedMaterials)
                        {
                            // DB (2021-05-07): SANITY CHECK
                            if (oMaterialSlot == null)
                            {
                                Debug.LogError("DB (2021-05-07), ERROR: MaterialSlot is NULL");
                                continue;
                            }
                            var sSlotName = oMaterialSlot.name;

                            Material nuMat = null;

                            if (_map != null && _map.Map.ContainsKey(sSlotName))
                            {
                                nuMat = _map.Map[sSlotName];// the preferred uber/iRay based material generated by the DTUConverter

                                // DB (2021-05-25): dForce import
                                if (_dforceMap.Map.ContainsKey(sSlotName))
                                {
                                    if (EnableDForceSupport)
                                        ImportDforceToPrefab(sSlotName, renderer, workingInstance, oMaterialSlot);

                                    //DForceMaterial dforceMat = _dforceMap.Map[sSlotName];
                                    ...
                                    //int matIndex = Array.IndexOf(skinned.sharedMaterials, oMaterialSlot);
                            ...
                            else if (s_StandardMaterialCollection.ContainsKey(sSlotName))
                            {
                                nuMat = new Material(s_StandardMaterialCollection[sSlotName]);
                                //FixupStandardBasedMaterial(ref nuMat, fbxPrefab, oMaterialSlot.name, data);
                            }
                            else
                            {
                                ...
                                nuMat.CopyPropertiesFromMaterial(oMaterialSlot);
                                ...
                                AssetDatabase.CreateAsset(nuMat, matPath + "/Daz3D_" + oMaterialSlot.name + ".mat");
                                */
                            }

                            dict.Add(oMaterialSlot, nuMat);
                        ...
                        for (int i = 0; i < count; i++)
                        {
                            ...
                            if (key == null || !dict.ContainsKey(key))
                            {
                                ...
                                if (key != null)
                                    Debug.LogError(" part 2: key==" + key + ", name=" + key.name);
                                else
            ...
            if (Daz3DBridge.BatchConversionMode != 0)
            {
                ...
                var offlinePrefab = PrefabUtility.SaveAsPrefabAssetAndConnect(workingInstance, nuPrefabPathPath, InteractionMode.AutomatedAction);
                Debug.Log("Prefab Created:" + offlinePrefab.name);
                return;
